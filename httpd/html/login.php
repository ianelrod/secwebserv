...
<body>
    <div class="d-flex align-items-center justify-content-center">
        <div class="p-2 border">
            ...
            $connection_string = "host={$host} port={$port} dbname={$db_name} user={$db_user} password={$db_pass}";
            $db_conn = pg_connect($connection_string);
            $query_sql = "SELECT * FROM users WHERE username=$1 AND password=crypt($2, password)";
            $query_prep = pg_prepare($db_conn, "login_query", $query_sql);
            ...
            if (isset($_POST['login']) && !empty($_POST['username']) && !empty($_POST['password'])) {
                ...
                $query_data = pg_execute($db_conn, "login_query", array(pg_escape_string($username), pg_escape_string($password)));
                $login_check = pg_num_rows($query_data);
